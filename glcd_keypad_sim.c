...
// ÊÚÑíİ íäåÇí ÓÊæä (ÎÑæÌí)
#define COL1_PIN 0
#define COL2_PIN 1
#define COL3_PIN 2

// ÊÚÑíİ íäåÇí ÓØÑ (æÑæÏí) - Èå ÕæÑÊ äÇãÑÊÈ
#define ROW1_PIN 7
#define ROW2_PIN 5
#define ROW3_PIN 6
#define ROW4_PIN 4
...
char get_key(void)
{
    ...
    // ÂÑÇíååÇíí ÈÑÇí ãÏíÑíÊ íäåÇí äÇãÑÊÈ
    ...
    // ÂÑÇíå ÈÑÇí äÇÔÊ ˜ÇÑÇ˜ÊÑåÇí ˜íÏ
    ...
    // ÍáŞå ÇÕáí: ÇÓ˜ä ÓÊæäåÇ
    for (col = 0; col < 3; col++)
    {
        // åãå ÓÊæäåÇ ÑÇ í˜ ãí˜äíã (íÇ Èå ÍÇáÊ ÇãÏÇäÓ ÈÇáÇ ãíÈÑíã)
        ...
        for (row = 0; row < 4; row++)
        {
            // ÇÑ í˜ íä ÓØÑ Èå ÎÇØÑ ÓÊæä İÚÇá¡ ÕİÑ ÔÏå ÈÇÔÏ
            if (!(KEYPAD_PIN & (1 << row_pins[row])))
            {
                ...
                if (!(KEYPAD_PIN & (1 << row_pins[row])))
                {
                    // ãäÊÙÑ ãíãÇäíã ÊÇ ˜ÇÑÈÑ ÏÓÊ ÎæÏ ÑÇ ÇÒ Ñæí ˜áíÏ ÈÑÏÇÑÏ
                    while (!(KEYPAD_PIN & (1 << row_pins[row])));
                    
                    // ˜ÇÑÇ˜ÊÑ ãÑÈæØå ÑÇ ÈÑãíÑÏÇäíã
...
void main(void)
{
    ...
    // ----- ÛíÑİÚÇá ˜ÑÏä JTAG (ÈÑÇí ÇÓÊİÇÏå ÇÒ æÑÊåÇí C æ F ÏÑ ÕæÑÊ äíÇÒ) -----
    MCUCSR = (1 << JTD);
    MCUCSR = (1 << JTD);

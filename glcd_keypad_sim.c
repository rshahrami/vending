...
unsigned char init_GPRS(void)
{
    ...
    // ÛíÑÝÚÇá ˜ÑÏä ÍÇáÊ ÎæÇÈ (Sleep Mode) 
    send_at_command("AT+CSCLK=0");   // ÖÑæÑí ÈÑÇí ÌáæíÑí ÇÒ ÞØÚí GPRS
    delay_ms(50);

    send_at_command("AT+CSMP=17,167,0,0"); // ÛíÑÝÚÇá ˜ÑÏä Sleep Mode
    delay_ms(100);

    // ÊäÙíã äæÚ ÇÊÕÇá Èå GPRS
    send_at_command("AT+SAPBR=3,1,\"Contype\",\"GPRS\""); 
    delay_ms(100);

    sprintf(at_command, "AT+SAPBR=3,1,\"APN\",\"%s\"", APN); // ÊäÙíã APN
    send_at_command(at_command);
    delay_ms(100);

    // ÛíÑÝÚÇá ˜ÑÏä Idle Timeout
    send_at_command("AT+SAPBR=3,1,\"Idle Timeout\",\"0\""); 
    delay_ms(150);

    // ÝÚÇáÓÇÒí Keep-Alive ÈÑÇí ÇÑÓÇá ˜íÊåÇí ÊÓÊ ÏæÑåÇí
    send_at_command("AT+CIPKEEPALIVE=1"); // ÇÑÓÇá ˜íÊ åÑ 20 ËÇäíå
    delay_ms(100);

    // ÇØãíäÇä ÇÒ ËÈÊäÇã ÎæÏ˜ÇÑ ÏÑ ÔÈ˜å (ÖÑæÑí ÈÑÇí Ñí˜Çä˜Ê ÎæÏ˜ÇÑ)
...
//    delay_ms(500); 
    ...
    while (1)
    {
        ...
        send_at_command("AT+PING=\"8.8.8.8\"");  // ÇÑÓÇá íä Èå Google DNS
        delay_ms(50);  // ÊÇÎíÑ Èíä ÇÑÓÇá ˜ÊåÇ (1 ËÇäíå)

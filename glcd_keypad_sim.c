...
#include <stdio.h>
#include <string.h>
#include <stdlib.h> // ÈÑÇí ÇÓÊİÇÏå ÇÒ ÊÇÈÚ atoi
//#include <stdint.h>
#include "bitmaps.h"
...
//#define SERVER_URL "http://google.com/api/authorize" // ÂÏÑÓ ˜Çãá ÓÑæÑ ÎæÏ ÑÇ ÇíäÌÇ ŞÑÇÑ ÏåíÏ
...
char response_buffer[256]; // ÇİÒÇíÔ ÓÇíÒ ÈÇİÑ ÈÑÇí ÏÑíÇİÊ ÇÓÎåÇí HTTP
...
unsigned char init_sms(void)
{
    ...
    // ÇØãíäÇä ÇÒ İÚÇá ÈæÏä ˜Çãá ãÇæá æ ÛíÑİÚÇá ÈæÏä sleep
    send_at_command("AT+CFUN=1");    // İÚÇáÓÇÒí ˜Çãá ãÇæá
    delay_ms(100);
    send_at_command("AT+CSCLK=0");   // ÛíÑİÚÇáÓÇÒí ÍÇáÊ sleep
    delay_ms(100);
...
//        glcd_outtextxy(0,10,response);  // äãÇíÔ ãÍÊæÇí response ˜Çãá ÈÑÇí ÏíÈÇ
//        delay_ms(500);
        return 0;
    }
...
void main(void)
{
    ...
    DDRD = (0 << DDD7) | (0 << DDD6) | (0 << DDD5) | (0 << DDD4) | (0 << DDD3) | (0 << DDD2) | (0 << DDD1) | (0 << DDD0);  // åãå æÑæÏí
    PORTD = (0 << PORTD7) | (0 << PORTD6) | (0 << PORTD5) | (0 << PORTD4) | (0 << PORTD3) | (0 << PORTD2) | (1 << PORTD1) | (0 << PORTD0); // Pull-down (íÚäí ÛíÑİÚÇá ˜ÑÏä pull-up)    
...
//    delay_ms(500); 
    ...
    while (1)
    {
        ...
        if (gets(header_buffer, sizeof(header_buffer)))
        {
            ...
            {
                // ?? İæÑÇğ ãÊä íÇã˜ Ñæ ÈÎæä æ äå ÏÇÑ
...
//                        delay_ms(200);
                        ...
                        sms_char = content_buffer[0];  // ãËáÇ '1'

...
// ÈÇİÑ ÈÑÇí ÏÑíÇİÊ ÏÇÏå ÇÒ ãÇæá
...
void main(void)
{
    ...
    // --- Çíä ÈÎÔ ãŞÏÇÑÏåí Çæáíå æÑÊåÇ æ USART ÇÓÊ ˜å ÇÒ ˜Ï ÎæÏÊÇä ˜í ÔÏå ---
    ...
    // 3. ÊäÙíã ãÇæá ÈÑÇí ÇÑÓÇá ãÓÊŞíã íÇã˜ Èå æÑÊ ÓÑíÇá
    ...
    while (1)
    {
        ...
        if (gets(header_buffer, sizeof(header_buffer)))
        {
            ...
            {
                // ÇÑ åÏÑ ÈæÏ¡ ÈÏæä åí ˜ÇÑ ÇÖÇİåÇí¡ ÈáÇİÇÕáå ÎØ ÈÚÏí (ãÍÊæÇ) ÑÇ ÈÎæÇä
                // Çíä ˜ÇÑ ÇÒ ÓÑÑíÒ ÔÏä ÈÇİÑ ÌáæíÑí ãí˜äÏ
                memset(content_buffer, 0, sizeof(content_buffer));
                gets(content_buffer, sizeof(content_buffer)); 

                // ÍÇáÇ ˜å åÑ Ïæ ÎØ ÏÑ ãÊÛíÑåÇí ãÇ Çãä åÓÊäÏ¡ ÈÇ ÎíÇá ÑÇÍÊ Ñæí äãÇíÔÑ äÔÇäÔÇä ÈÏå
                glcd_clear();
                ...
                // äÏ ËÇäíå íÇã ÑÇ äå ÏÇÑ ÊÇ ÎæÇäÏå ÔæÏ
                delay_ms(10000); 

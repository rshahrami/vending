...
_active_GPRS:
; .FSTART _active_GPRS
; 0000 00E3     char at_command[50];
; 0000 00E4     char response[100]; // Local buffer for the response
; 0000 00E5 
; 0000 00E6 //    glcd_clear();
; 0000 00E7 //    glcd_outtextxy(0, 0, "Connecting to GPRS...");
; 0000 00E8 
; 0000 00E9     send_at_command("AT+SAPBR=3,1,\"Contype\",\"GPRS\"");
	SBIW R28,63
	SBIW R28,63
	SBIW R28,24
;	at_command -> Y+100
...
; 0000 00EB 
; 0000 00EC     sprintf(at_command, "AT+SAPBR=3,1,\"APN\",\"%s\"", APN);
	CALL SUBOPT_0xA
	CALL SUBOPT_0xB
; 0000 00ED     send_at_command(at_command);
	CALL SUBOPT_0x6
; 0000 00EE     delay_ms(200);
; 0000 00EF 
; 0000 00F0     send_at_command("AT+SAPBR=1,1");
	__POINTW2MN _0x1F,30
	CALL SUBOPT_0x6
; 0000 00F1     delay_ms(200);
; 0000 00F2 
; 0000 00F3     // If we reach here, it means getting the IP address failed
; 0000 00F4     return 0; // Failure
_0x2120013:
	LDI  R30,LOW(0)
_0x2120014:
	ADIW R28,63
...
; 0000 00FA unsigned char send_json_post(const char* base_url, const char* phone_number) {

	.CSEG
_send_json_post:
; .FSTART _send_json_post
; 0000 00FB     char cmd[256];
; 0000 00FC     char response[256];
; 0000 00FD     char full_url[256];
; 0000 00FE     char* action_ptr;
; 0000 00FF     int method = 0, status_code = 0, data_len = 0;
; 0000 0100 
...
;	data_len -> Y+6
	...
	__GETWRN 20,21,0
	CALL SUBOPT_0x4
; 0000 0102     draw_bitmap(0, 0, lotfan_montazer_bemanid, 128, 64);
	LDI  R30,LOW(_lotfan_montazer_bemanid*2)
	LDI  R31,HIGH(_lotfan_montazer_bemanid*2)
	CALL SUBOPT_0xF
; 0000 0103 
; 0000 0104 
; 0000 0105     active_GPRS();
	RCALL _active_GPRS
; 0000 0106 
; 0000 0107 
; 0000 0108 //    if (!check_gprs_status()) {
; 0000 0109 //        if (!active_GPRS()) {
; 0000 010A //            glcd_clear();
; 0000 010B //            glcd_outtextxy(0, 0, "GPRS Failed!");
...
; 0000 0113 //    glcd_outtextxy(0,10,phone_number);
; 0000 0114 //    glcd_outtextxy(0,20,"lev 0");
; 0000 0115 
; 0000 0116     // 1. Initialize HTTP service
; 0000 0117     send_at_command("AT+HTTPINIT");
	__POINTW2MN _0x20,0
	CALL SUBOPT_0x10
; 0000 0118     if (!read_serial_response(response, sizeof(response), 150, "OK")) return 0;
...
; 0000 011B //    glcd_outtextxy(0,10,phone_number);
; 0000 011C //    glcd_outtextxy(0,20,"lev 1");
; 0000 011D 
; 0000 011E     // 2. Set CID to bearer profile 1
; 0000 011F     send_at_command("AT+HTTPPARA=\"CID\",1");
_0x21:
	__POINTW2MN _0x20,15
	CALL SUBOPT_0x10
; 0000 0120     if (!read_serial_response(response, sizeof(response), 150, "OK")) return 0;
	__POINTW2MN _0x20,35
	RCALL _read_serial_response
	CPI  R30,0
	BRNE _0x22
	LDI  R30,LOW(0)
	RJMP _0x2120012
; 0000 0121 
; 0000 0122 //    glcd_outtextxy(0,20,"lev 2");
; 0000 0123 
; 0000 0124     // 3. Build the full URL with query parameter
; 0000 0125     sprintf(full_url, "%s?phone_number=%s", base_url, phone_number);
_0x22:
	MOVW R30,R28
	ADIW R30,8
	ST   -Y,R31
	...
	ADIW R28,12
; 0000 0126 
; 0000 0127     // 4. Set the target URL
; 0000 0128     sprintf(cmd, "AT+HTTPPARA=\"URL\",\"%s\"", full_url);
	MOVW R30,R28
	SUBI R30,LOW(-(520))
	SBCI R31,HIGH(-(520))
	ST   -Y,R31
	...
	CALL _sprintf
	ADIW R28,8
; 0000 0129     send_at_command(cmd);
	MOVW R26,R28
	SUBI R26,LOW(-(520))
	SBCI R27,HIGH(-(520))
	CALL SUBOPT_0x10
; 0000 012A     if (!read_serial_response(response, sizeof(response), 150, "OK")) return 0;
	__POINTW2MN _0x20,38
	RCALL _read_serial_response
	CPI  R30,0
	BRNE _0x23
	LDI  R30,LOW(0)
	RJMP _0x2120012
; 0000 012B 
; 0000 012C //    glcd_outtextxy(0,20,"lev 3");
; 0000 012D 
; 0000 012E     // 5. Start POST action
; 0000 012F     send_at_command("AT+HTTPACTION=1");
_0x23:
	__POINTW2MN _0x20,41
	RCALL _send_at_command
; 0000 0130     if (!read_serial_response(response, sizeof(response), HTTP_TIMEOUT_MS, "+HTTPACTION:")) {
	MOVW R30,R28
	SUBI R30,LOW(-(264))
	SBCI R31,HIGH(-(264))
	ST   -Y,R31
	...
	ST   -Y,R31
	ST   -Y,R30
	__POINTW2MN _0x20,57
	RCALL _read_serial_response
	CPI  R30,0
	BRNE _0x24
; 0000 0131         glcd_clear();
	CALL SUBOPT_0x4
; 0000 0132         glcd_outtextxy(0,0,"No Action Resp");
	__POINTW2MN _0x20,70
	CALL _glcd_outtextxy
; 0000 0133         delay_ms(100);
	LDI  R26,LOW(100)
	LDI  R27,0
	CALL _delay_ms
; 0000 0134         return 0;
...
; 0000 0135     }
; 0000 0136 
; 0000 0137 //    glcd_outtextxy(0,20,"lev 4");
; 0000 0138 
; 0000 0139     // 6. Parse status code from response
; 0000 013A 
; 0000 013B     action_ptr = strstr(response, "+HTTPACTION:");
_0x24:
	MOVW R30,R28
	SUBI R30,LOW(-(264))
	SBCI R31,HIGH(-(264))
	ST   -Y,R31
	ST   -Y,R30
	__POINTW2MN _0x20,85
	CALL _strstr
	MOVW R16,R30
; 0000 013C     if (action_ptr == NULL || sscanf(action_ptr, "+HTTPACTION: %d,%d,%d", &method, &status_code, &data_len) != 3) {
	CLR  R0
	CP   R0,R16
	CPC  R0,R17
	BREQ _0x26
...
; 0000 013E //        glcd_outtextxy(0,0,"Parse Err");
; 0000 013F //        glcd_outtextxy(0,10,response);  // ‰„«Ì‘ „Õ Ê«Ì response ò«„· »—«Ì œÌ»«ê
...
; 0000 0142     }
; 0000 0143 
; 0000 0144 //    glcd_outtextxy(0,20,"lev 5");
; 0000 0145     //delay_ms(200);
; 0000 0146     // 7. Show response based on status
; 0000 0147 //    glcd_clear();
; 0000 0148 //    glcd_outtextxy(0,0,"HTTP Status:");
; 0000 0149 //    if (status_code == 200) {
; 0000 014A //        glcd_outtextxy(0,10,"OK");
; 0000 014B //    } else {
; 0000 014C //        glcd_outtextxy(0,10,"Not OK");
; 0000 014D //    }
; 0000 014E     //delay_ms(200);
; 0000 014F //    glcd_outtextxy(0,20,"lev 6");
; 0000 0150 
; 0000 0151     // 8. Read server response if needed
; 0000 0152     send_at_command("AT+HTTPREAD");
_0x25:
	__POINTW2MN _0x20,98
	CALL SUBOPT_0x10
; 0000 0153     read_serial_response(response, sizeof(response), 150, "OK");
	__POINTW2MN _0x20,110
	RCALL _read_serial_response
; 0000 0154 
; 0000 0155 //    glcd_outtextxy(0,20,"lev 7");
; 0000 0156 
; 0000 0157     // 9. Terminate HTTP service
; 0000 0158     send_at_command("AT+HTTPTERM");
	__POINTW2MN _0x20,113
	CALL SUBOPT_0x10
; 0000 0159     read_serial_response(response, sizeof(response), 150, "OK");
	__POINTW2MN _0x20,125
	RCALL _read_serial_response
; 0000 015A 
; 0000 015B //    glcd_outtextxy(0,20,"lev 8");
; 0000 015C     return (status_code == 200) ? 1 : 0;
	LDI  R30,LOW(200)
	LDI  R31,HIGH(200)
	CP   R30,R20
	CPC  R31,R21
...
_get_key:
; .FSTART _get_key
; 0000 0163     unsigned char row, col;
; 0000 0164     const unsigned char column_pins[3] = {COL1_PIN, COL2_PIN, COL3_PIN};
; 0000 0165     const unsigned char row_pins[4] = {ROW1_PIN, ROW2_PIN, ROW3_PIN, ROW4_PIN};
; 0000 0166 
; 0000 0167     const char key_map[4][3] = {
; 0000 0168         {'1', '2', '3'},
; 0000 0169         {'4', '5', '6'},
; 0000 016A         {'7', '8', '9'},
; 0000 016B         {'*', '0', '#'}
...
_0x2D:
	...
	BRSH _0x2E
; 0000 016F     {
; 0000 0170         KEYPAD_PORT |= (1 << COL1_PIN) | (1 << COL2_PIN) | (1 << COL3_PIN);
	IN   R30,0x15
	ORI  R30,LOW(0x7)
	OUT  0x15,R30
; 0000 0171         KEYPAD_PORT &= ~(1 << column_pins[col]);
	IN   R1,21
	MOV  R30,R16
	LDI  R31,0
	MOVW R26,R28
	...
	OUT  0x15,R30
; 0000 0172 
; 0000 0173         for (row = 0; row < 4; row++)
	LDI  R17,LOW(0)
_0x30:
	CPI  R17,4
	BRSH _0x31
; 0000 0174         {
; 0000 0175             if (!(KEYPAD_PIN & (1 << row_pins[row])))
	CALL SUBOPT_0x13
	MOV  R26,R1
	CALL SUBOPT_0x1
	BRNE _0x32
; 0000 0176             {
; 0000 0177                 delay_ms(10);
	LDI  R26,LOW(10)
	LDI  R27,0
	CALL _delay_ms
; 0000 0178 
; 0000 0179                 if (!(KEYPAD_PIN & (1 << row_pins[row])))
	CALL SUBOPT_0x13
	MOV  R26,R1
	CALL SUBOPT_0x1
	BRNE _0x33
; 0000 017A                 {
; 0000 017B                     while (!(KEYPAD_PIN & (1 << row_pins[row])));
_0x34:
	CALL SUBOPT_0x13
	MOV  R26,R1
	CALL SUBOPT_0x1
	BREQ _0x34
; 0000 017C                     return key_map[row][col];
	LDI  R26,LOW(3)
	MUL  R17,R26
	MOVW R30,R0
	MOVW R26,R28
...
_0x2E:
; 0000 0181 
; 0000 0182     // C?? ??? ???I? ?O?I? ?OI? E?I? ??IC? ??? (NULL) ?C E???IC?
; 0000 0183     return 0;
	LDI  R30,LOW(0)
_0x2120011:
	LDD  R17,Y+1
...
; 0000 0189     unsigned char motor_pin;
; 0000 018A     char motor_msg[20];
; 0000 018B     int timeout = 5000;
; 0000 018C 
; 0000 018D     switch (product_id)
	ST   -Y,R27
	ST   -Y,R26
	SBIW R28,20
	CALL __SAVELOCR4
...
;	timeout -> R18,R19
	...
	LDD  R31,Y+24+1
; 0000 018E     {
; 0000 018F         case 1: motor_pin = MOTOR_PIN_1; break;
	CPI  R30,LOW(0x1)
	LDI  R26,HIGH(0x1)
	CPC  R31,R26
	BRNE _0x3A
	LDI  R17,LOW(2)
	RJMP _0x39
; 0000 0190         case 2: motor_pin = MOTOR_PIN_2; break;
_0x3A:
	CPI  R30,LOW(0x2)
	LDI  R26,HIGH(0x2)
	CPC  R31,R26
	...
	LDI  R17,LOW(3)
	RJMP _0x39
; 0000 0191         case 3: motor_pin = MOTOR_PIN_3; break;
_0x3B:
	CPI  R30,LOW(0x3)
	LDI  R26,HIGH(0x3)
	CPC  R31,R26
	...
	LDI  R17,LOW(4)
	RJMP _0x39
; 0000 0192         default: return;
_0x3D:
	RJMP _0x2120010
; 0000 0193     }
_0x39:
; 0000 0194 
; 0000 0195     sprintf(motor_msg, "MOTOR %d ON!", product_id);
	MOVW R30,R28
	ADIW R30,4
	ST   -Y,R31
	ST   -Y,R30
	...
	CALL _sprintf
	ADIW R28,8
; 0000 0196     glcd_clear();
	CALL _glcd_clear
; 0000 0197     glcd_outtextxy(10, 20, motor_msg);
	LDI  R30,LOW(10)
	ST   -Y,R30
	LDI  R30,LOW(20)
	ST   -Y,R30
	...
	ADIW R26,6
	CALL _glcd_outtextxy
; 0000 0198     MOTOR_PORT |= (1 << motor_pin);
	IN   R1,3
	MOV  R30,R17
	LDI  R26,LOW(1)
	CALL __LSLB12
	...
	OUT  0x3,R30
; 0000 0199 
; 0000 019A //    while (!(PIND & (1 << PIND1)) && timeout > 0)
; 0000 019B //    {
; 0000 019C //        delay_ms(1);
; 0000 019D //        timeout--;
; 0000 019E //    }
; 0000 019F 
; 0000 01A0     while ((PIND & (1 << PIND1)) && timeout > 0)
_0x3E:
	SBIS 0x10,1
	RJMP _0x41
	CLR  R0
...
_0x40:
; 0000 01A5 
; 0000 01A6     MOTOR_PORT &= ~(1 << motor_pin);
	IN   R1,3
	MOV  R30,R17
	LDI  R26,LOW(1)
	CALL SUBOPT_0x12
	OUT  0x3,R30
; 0000 01A7 
; 0000 01A8 //    sprintf(motor_msg, "MOTOR %d OFF!", product_id);
; 0000 01A9 //    glcd_outtextxy(10, 40, motor_msg);
; 0000 01AA     draw_bitmap(0, 0, mahsol_ra_bardarid, 128, 64);
	LDI  R30,LOW(0)
	ST   -Y,R30
	ST   -Y,R30
...
_main:
; .FSTART _main
; 0000 01B2     const char* server_url = "http://193.5.44.191/home/post/";
; 0000 01B3     // const char* my_phone    = "+989152608582";
; 0000 01B4 
; 0000 01B5     GLCDINIT_t glcd_init_data;
; 0000 01B6 
; 0000 01B7     // --- C?? EIO ??IC?I?? C???? ???E??C ? USART C?E ?? C? ?I I?IEC? ??? OI? ---
; 0000 01B8     // --- ? ???? C?E. ??C?? E? EU??? A? ???E.                             ---
; 0000 01B9     DDRA=(0<<DDA7) | (0<<DDA6) | (0<<DDA5) | (0<<DDA4) | (0<<DDA3) | (0<<DDA2) | (0<<DDA1) | (0<<DDA0);
	SBIW R28,6
;	*server_url -> R16,R17
;	glcd_init_data -> Y+0
...
; 0000 01BA     PORTA=(0<<PORTA7) | (0<<PORTA6) | (0<<PORTA5) | (0<<PORTA4) | (0<<PORTA3) | (0<<PORTA2) | (0<<PORTA1) | (0<<PORTA0);
	OUT  0x1B,R30
; 0000 01BB     DDRB=(0<<DDB7) | (0<<DDB6) | (0<<DDB5) | (0<<DDB4) | (0<<DDB3) | (0<<DDB2) | (0<<DDB1) | (0<<DDB0);
	OUT  0x17,R30
; 0000 01BC     PORTB=(1<<PORTB7) | (1<<PORTB6) | (1<<PORTB5) | (1<<PORTB4) | (0<<PORTB3) | (0<<PORTB2) | (0<<PORTB1) | (0<<PORTB0);
	LDI  R30,LOW(240)
	OUT  0x18,R30
; 0000 01BD     DDRC=(0<<DDC7) | (0<<DDC6) | (0<<DDC5) | (0<<DDC4) | (0<<DDC3) | (1<<DDC2) | (1<<DDC1) | (1<<DDC0);
	LDI  R30,LOW(7)
	OUT  0x14,R30
; 0000 01BE     PORTC=(1<<PORTC7) | (1<<PORTC6) | (1<<PORTC5) | (1<<PORTC4) | (0<<PORTC3) | (0<<PORTC2) | (0<<PORTC1) | (0<<PORTC0);
	LDI  R30,LOW(240)
	OUT  0x15,R30
; 0000 01BF 
; 0000 01C0     DDRD = (0 << DDD7) | (0 << DDD6) | (0 << DDD5) | (0 << DDD4) | (0 << DDD3) | (0 << DDD2) | (0 << DDD1) | (0 << DDD0) ...
	LDI  R30,LOW(0)
	OUT  0x11,R30
; 0000 01C1     PORTD = (0 << PORTD7) | (0 << PORTD6) | (0 << PORTD5) | (0 << PORTD4) | (0 << PORTD3) | (0 << PORTD2) | (1 << PORTD1 ...
	LDI  R30,LOW(2)
	OUT  0x12,R30
; 0000 01C2 
; 0000 01C3     DDRE=(0<<DDE7) | (0<<DDE6) | (1<<DDE5) | (1<<DDE4) | (1<<DDE3) | (1<<DDE2) | (0<<DDE1) | (0<<DDE0);
	LDI  R30,LOW(60)
	OUT  0x2,R30
; 0000 01C4     PORTE=(0<<PORTE7) | (0<<PORTE6) | (0<<PORTE5) | (0<<PORTE4) | (0<<PORTE3) | (0<<PORTE2) | (0<<PORTE1) | (0<<PORTE0);
	LDI  R30,LOW(0)
	OUT  0x3,R30
; 0000 01C5     DDRF=(0<<DDF7) | (0<<DDF6) | (0<<DDF5) | (0<<DDF4) | (0<<DDF3) | (0<<DDF2) | (0<<DDF1) | (0<<DDF0);
	STS  97,R30
; 0000 01C6     PORTF=(0<<PORTF7) | (0<<PORTF6) | (0<<PORTF5) | (0<<PORTF4) | (0<<PORTF3) | (0<<PORTF2) | (0<<PORTF1) | (0<<PORTF0);
	STS  98,R30
; 0000 01C7     UCSR0A=(0<<RXC0) | (0<<TXC0) | (0<<UDRE0) | (0<<FE0) | (0<<DOR0) | (0<<UPE0) | (0<<U2X0) | (0<<MPCM0);
	OUT  0xB,R30
; 0000 01C8     UCSR0B=(0<<RXCIE0) | (0<<TXCIE0) | (0<<UDRIE0) | (1<<RXEN0) | (1<<TXEN0) | (0<<UCSZ02) | (0<<RXB80) | (0<<TXB80);
	LDI  R30,LOW(24)
	OUT  0xA,R30
; 0000 01C9     UCSR0C=(0<<UMSEL0) | (0<<UPM01) | (0<<UPM00) | (0<<USBS0) | (1<<UCSZ01) | (1<<UCSZ00) | (0<<UCPOL0);
	LDI  R30,LOW(6)
	STS  149,R30
; 0000 01CA     UBRR0H=0x00;
	LDI  R30,LOW(0)
	STS  144,R30
; 0000 01CB     UBRR0L=0x33; // 9600 Baud Rate for 8MHz clock
	LDI  R30,LOW(51)
	OUT  0x9,R30
; 0000 01CC     MCUCSR = (1 << JTD);
	LDI  R30,LOW(128)
	OUT  0x34,R30
; 0000 01CD     MCUCSR = (1 << JTD);
	OUT  0x34,R30
; 0000 01CE     glcd_init_data.font=font5x7;
	LDI  R30,LOW(_font5x7*2)
	LDI  R31,HIGH(_font5x7*2)
	ST   Y,R30
	STD  Y+1,R31
; 0000 01CF     glcd_init_data.readxmem=NULL;
	LDI  R30,LOW(0)
	STD  Y+2,R30
	STD  Y+2+1,R30
; 0000 01D0     glcd_init_data.writexmem=NULL;
	STD  Y+4,R30
	STD  Y+4+1,R30
; 0000 01D1     glcd_init(&glcd_init_data);
	MOVW R26,R28
	RCALL _glcd_init
; 0000 01D2     glcd_setfont(font5x7);
	LDI  R30,LOW(_font5x7*2)
	LDI  R31,HIGH(_font5x7*2)
	CALL SUBOPT_0x15
; 0000 01D3     // --- ?C?C? EIO ??IC?I?? C???? ---
; 0000 01D4 
; 0000 01D5     glcd_clear();
	CALL SUBOPT_0x4
; 0000 01D6     glcd_outtextxy(0, 0, "Module Init...");
	__POINTW2MN _0x43,31
	CALL _glcd_outtextxy
; 0000 01D7     delay_ms(3000);
	...
	CALL _delay_ms
; 0000 01D8 
; 0000 01D9     send_at_command("ATE0");
	__POINTW2MN _0x43,46
	CALL SUBOPT_0x9
; 0000 01DA     delay_ms(300);
; 0000 01DB     send_at_command("AT");
	__POINTW2MN _0x43,51
	CALL SUBOPT_0x9
; 0000 01DC     delay_ms(300);
; 0000 01DD 
; 0000 01DE     if (!init_sms()) { glcd_outtextxy(0, 10, "SMS Init Failed!"); while(1); }
	RCALL _init_sms
	CPI  R30,0
	BRNE _0x44
	CALL SUBOPT_0x7
	__POINTW2MN _0x43,54
	CALL _glcd_outtextxy
_0x45:
	RJMP _0x45
; 0000 01DF     if (!init_GPRS()) { glcd_outtextxy(0, 10, "GPRS Init Failed!"); while(1); }
_0x44:
	RCALL _init_GPRS
	CPI  R30,0
	BRNE _0x48
...
; 0000 01E3 //    draw_bitmap(0, 0, adad_ra_vared_namaeid, 128, 64);
...
_0x48:
	CALL SUBOPT_0x4
; 0000 01E8     draw_bitmap(0, 0, shomare_soton_ra_payamak, 128, 64);
...
_0x4C:
; 0000 01EC     {
; 0000 01ED         char sms_char;
; 0000 01EE         char key_pressed;
; 0000 01EF         char display_buffer[2] = {0};
; 0000 01F0         int product_id = 0;
; 0000 01F1         int timeout_counter = 0;
; 0000 01F2         char* token;
; 0000 01F3 
; 0000 01F4         char content_buffer[32];
; 0000 01F5         memset(header_buffer, 0, sizeof(header_buffer));
	SBIW R28,42
	LDI  R30,LOW(0)
	STD  Y+34,R30
	STD  Y+35,R30
...
;	content_buffer -> Y+0
	...
	LDI  R27,0
	CALL _memset
; 0000 01F6         memset(content_buffer, 0, sizeof(content_buffer));
	MOVW R30,R28
	CALL SUBOPT_0x3
	LDI  R26,LOW(32)
	LDI  R27,0
	CALL _memset
; 0000 01F7 
; 0000 01F8         if (gets(header_buffer, sizeof(header_buffer)))
	CALL SUBOPT_0x16
	LDI  R26,LOW(100)
	LDI  R27,0
	CALL _gets
	...
	RJMP _0x4F
; 0000 01F9         {
; 0000 01FA             if (strstr(header_buffer, "+CMT:") != NULL)
	CALL SUBOPT_0x16
	__POINTW2MN _0x43,89
	CALL _strstr
	SBIW R30,0
	BRNE PC+2
	RJMP _0x50
; 0000 01FB             {
; 0000 01FC                 // ?? ›Ê—« „ ‰ ÅÌ«„ò —Ê »ŒÊ‰ Ê ‰êÂ œ«—
; 0000 01FD                 if (!gets(content_buffer, sizeof(content_buffer))) {
	CALL SUBOPT_0xC
	LDI  R26,LOW(32)
	LDI  R27,0
	CALL _gets
	SBIW R30,0
	BRNE _0x51
; 0000 01FE                     glcd_clear();
	CALL SUBOPT_0x4
; 0000 01FF                     glcd_outtextxy(0, 0, "Failed to read msg");
	__POINTW2MN _0x43,95
	CALL _glcd_outtextxy
; 0000 0200                     delay_ms(1000);
	LDI  R26,LOW(1000)
	LDI  R27,HIGH(1000)
	CALL _delay_ms
; 0000 0201                     continue;
...
; 0000 0203 
; 0000 0204                 // ?? ‘„«—Â  ·›‰ —Ê »êÌ—
; 0000 0205                 token = strtok(header_buffer, "\"");
_0x51:
	CALL SUBOPT_0x16
	CALL SUBOPT_0x17
; 0000 0206                 if (token != NULL) token = strtok(NULL, "\"");
	LDD  R30,Y+32
	LDD  R31,Y+32+1
	SBIW R30,0
	BREQ _0x52
...
_0x52:
	...
	RJMP _0x53
; 0000 0208                 {
; 0000 0209                     strcpy(phone_number, token);
	...
	CALL _strcpy
; 0000 020A 
; 0000 020B                     // ?? Õ«·« „ÃÊ“ —Ê »——”Ì ò‰
; 0000 020C                     if (send_json_post(server_url, phone_number))
	ST   -Y,R17
	ST   -Y,R16
	...
	RJMP _0x54
; 0000 020D                     {
; 0000 020E //                        glcd_clear();
; 0000 020F //                        glcd_outtextxy(0, 5, "step 1");
; 0000 0210 //                        delay_ms(200);
; 0000 0211 
; 0000 0212                         sms_char = content_buffer[0];  // „À·« '1'
	LD   R30,Y
	STD  Y+41,R30
; 0000 0213 
; 0000 0214 //                        glcd_outtextxy(0, 5, "step 2");
; 0000 0215 //                        delay_ms(200);
; 0000 0216 
; 0000 0217                         if (sms_char == '1' || sms_char == '2' || sms_char == '3')
	LDD  R26,Y+41
	CPI  R26,LOW(0x31)
	BREQ _0x56
	CPI  R26,LOW(0x32)
...
; 0000 0219                             glcd_clear();
	CALL SUBOPT_0x4
; 0000 021A //                            glcd_outtextxy(0, 5, "SMS Code:");
; 0000 021B                             draw_bitmap(0, 0, square, 128, 32);
	LDI  R30,LOW(_square*2)
	LDI  R31,HIGH(_square*2)
	CALL SUBOPT_0x19
; 0000 021C                             display_buffer[0] = sms_char;
	LDD  R30,Y+41
	STD  Y+38,R30
; 0000 021D                             glcd_outtextxy(65, 10, display_buffer);
	LDI  R30,LOW(65)
	ST   -Y,R30
	LDI  R30,LOW(10)
	ST   -Y,R30
	...
	ADIW R26,40
	CALL SUBOPT_0xD
; 0000 021E //                            glcd_outtextxy(0, 25, "Enter code on keypad:");
; 0000 021F                             draw_bitmap(0, 20, adad_ra_vared_namaeid, 128, 64);
	LDI  R30,LOW(_adad_ra_vared_namaeid*2)
	LDI  R31,HIGH(_adad_ra_vared_namaeid*2)
	CALL SUBOPT_0xF
; 0000 0220 
; 0000 0221                             key_pressed = 0;
	LDI  R30,LOW(0)
	STD  Y+40,R30
; 0000 0222                             for (timeout_counter = 0; timeout_counter < 200; timeout_counter++)
	STD  Y+34,R30
	STD  Y+34+1,R30
_0x59:
	LDD  R26,Y+34
	...
	BRGE _0x5A
; 0000 0223                             {
; 0000 0224                                 key_pressed = get_key();
	RCALL _get_key
	STD  Y+40,R30
; 0000 0225                                 if (key_pressed != 0) break;
	CPI  R30,0
	BRNE _0x5A
; 0000 0226                                 delay_ms(10);
	LDI  R26,LOW(10)
	LDI  R27,0
	CALL _delay_ms
; 0000 0227                             }
...
_0x5A:
; 0000 0228 
; 0000 0229                             if (key_pressed == 0)
	LDD  R30,Y+40
	CPI  R30,0
	BRNE _0x5C
; 0000 022A                             {
; 0000 022B                                 glcd_clear();
	RCALL _glcd_clear
; 0000 022C                                 glcd_outtextxy(10, 25, "Timeout! Try again.");
	LDI  R30,LOW(10)
	ST   -Y,R30
	LDI  R30,LOW(25)
	ST   -Y,R30
	__POINTW2MN _0x43,114
	CALL _glcd_outtextxy
...
_0x5C:
; 0000 0230                             {
; 0000 0231 //                                glcd_outtextxy(0, 45, "You pressed:");
; 0000 0232                                 //draw_bitmap(70, 45, square, 128, 32);
; 0000 0233 //                                draw_bitmap(65, 0, square, 128, 32);
; 0000 0234                                 display_buffer[0] = key_pressed;
	LDD  R30,Y+40
	STD  Y+38,R30
; 0000 0235                                 glcd_outtextxy(65, 50, display_buffer);
	LDI  R30,LOW(65)
	ST   -Y,R30
	LDI  R30,LOW(50)
	ST   -Y,R30
	...
	CALL SUBOPT_0x8
; 0000 0236 
; 0000 0237                                 delay_ms(200);
; 0000 0238 
; 0000 0239                                 if (key_pressed == sms_char)
	LDD  R30,Y+41
	LDD  R26,Y+40
	CP   R30,R26
	BRNE _0x5E
...
; 0000 023B                                     glcd_clear();
	RCALL _glcd_clear
; 0000 023C //                                    draw_bitmap(0, 0, adad_ra_vared_namaeid, 128, 64);
; 0000 023D                                     //glcd_outtextxy(10, 25, "Code is CORRECT!");
; 0000 023E                                     //draw_bitmap(0, 0, shomare_soton_dorost_vared_nashode, 128, 32);
; 0000 023F                                     //delay_ms(300);
; 0000 0240                                     product_id = sms_char - '0';
	LDD  R30,Y+41
	LDI  R31,0
	SBIW R30,48
	STD  Y+36,R30
	STD  Y+36+1,R31
; 0000 0241                                     activate_motor(product_id);
	LDD  R26,Y+36
	LDD  R27,Y+36+1
...
; 0000 0245                                     glcd_clear();
	CALL SUBOPT_0x4
; 0000 0246 //                                    glcd_outtextxy(5, 25, "Error in entry!");
; 0000 0247                                     draw_bitmap(0, 0, shomare_soton_dorost_vared_nashode, 128, 32);
...
; 0000 024E                             glcd_clear();
	CALL SUBOPT_0x4
; 0000 024F //                            glcd_outtextxy(5, 25, "Invalid SMS Code!");
; 0000 0250                             draw_bitmap(0, 0, shomare_dorost_payamak_nashode, 128, 64);
...
; 0000 0256                         glcd_clear();
	CALL SUBOPT_0x4
; 0000 0257 //                        glcd_outtextxy(0, 25, "You are not authorized!");
; 0000 0258                         draw_bitmap(0, 0, tedad_bish_az_had, 128, 64);
...
; 0000 025B                 }
; 0000 025C 
; 0000 025D                 // »«“ê‘  »Â Ê÷⁄Ì  ¬„«œÂù»Âùò«—
; 0000 025E                 glcd_clear();
_0x53:
	CALL SUBOPT_0x4
; 0000 025F                 draw_bitmap(0, 0, shomare_soton_ra_payamak, 128, 64);
	LDI  R30,LOW(_shomare_soton_ra_payamak*2)
	LDI  R31,HIGH(_shomare_soton_ra_payamak*2)
	CALL SUBOPT_0xF
; 0000 0260 //                glcd_outtextxy(0, 0, "System Ready.");
; 0000 0261 //                glcd_outtextxy(0, 10, "Waiting for SMS...");
; 0000 0262 
; 0000 0263             }
; 0000 0264         }
_0x50:
